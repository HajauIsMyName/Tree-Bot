...
class EventHandlers( commands.Cog ):
    ...
    async def on_command_error(self, ctx, error):
        if isinstance( error, commands.CommandOnCooldown ):
            ...
            if cooldown >= 3600:
                msg = f"**:stopwatch: | {ctx.author.name}**! Please wait **{cooldown // 3600}**h **{int( (cooldown % 3600) // 60 )}**m and try again!"

            elif cooldown >= 60:
                msg = f"**:stopwatch: | {ctx.author.name}**! Please wait **{int( (cooldown % 3600) // 60 )}**m **{round( cooldown % 60, 2 )}**s and try again!"

            else:
                msg = f"**:stopwatch: | {ctx.author.name}**! Please wait **{round(error.retry_after % 60, 2)}** and try again!"
    ...
    Need more help? Come join our [guild](https://discord.gg/YWXNzxJAxz)""", color=discord.Color.red() )
        embed.set_author( name="Command List",
                          icon_url=ctx.author.avatar_url )
        embed.add_field(
            name="💰 Economy", value="``balance``, ``beg``, ``deposit``, ``give``, ``withdraw``, ``work``",
            inline=False )
        embed.add_field( name="🙂 Emotes",
                         value="``hug``, ``kiss``, ``slap``", inline=False )
        embed.add_field( name="🎲 Gambling", value="``coinflip``", inline=False )
        embed.add_field(
            name="🔧 Utility", value="``avatar``, ``echo``, ``emojify``, ``invite``, ``ping``, ``python``, ``server``",
            inline=False )
...
async def setup(client):
    await client.add_cog( EventHandlers( client ) )
