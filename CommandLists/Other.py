...
class Other( commands.Cog ):
    ...
    async def invite(self, ctx):
        embed = discord.Embed( title=f"Invite {self.client.user.name}", color=0xff0000,
            ...
            components=[
                ActionRow( [
                    Button(
                        ...
                        label="Invite",
                        url=f"https://discord.com/api/oauth2/authorize?client_id={self.client.user.id}&permissions=8&scope=bot"
                    )
                ] )
            ]
        )
    ...
    async def server(self, ctx):
        ...
        await ctx.send( f"Here's an invite for BrehsServer's support guild: {url}" )
